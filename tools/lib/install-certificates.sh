...
install_certificates_on_host() {
    ...
    # Retrieve Caddy's server certificate (generated by tls internal)
    log_info "  Retrieving Caddy SSL certificate from VM..."
    if ssh -i "$VM_SSH_PRIVATE_KEY" -p "$VM_SSH_PORT" \
        -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null \
        -o ConnectTimeout=10 \
        ${VM_USERNAME}@localhost \
        "doas cat /var/lib/caddy/.local/share/caddy/certificates/local/${VM_HOSTNAME}/${VM_HOSTNAME}.crt" > "$cert_file" 2>/dev/null; then
        
        log_success "  ✓ Caddy SSL certificate retrieved from VM"
        ...
        if sudo cp "$cert_file" /usr/local/share/ca-certificates/trustnet-caddy.crt 2>/dev/null; then
            log_info "    Installing Caddy certificate to system trust store..."
            sudo update-ca-certificates >/dev/null 2>&1
            
            if [ -f /usr/local/share/ca-certificates/trustnet-caddy.crt ]; then
                log_success "  ✓ Caddy certificate installed to system trust store"
                cert_installed=true
            fi
        else
